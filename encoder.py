...
class IVFPQEncoder:
    ...
    """
    def __init__(self, models_dir: str, m_subvectors: int):
    ...
    def encode_dataset(self, X: np.ndarray) -> tuple:
        ...
        # Use uint8 because k_subcentroids = 256, which fits perfectly in 1 byte
        pq_codes = np.zeros((num_vectors, self.m_subvectors), dtype=np.uint8)
        ...
        for m in range(self.m_subvectors):
            ...
            codebook_subspace = self.pq_codebooks[m] # Shape: (256, sub_dim)
