...
int main() {
    ...
    std::ifstream in_tw("../../../integration_tools/test_vectors/inv_twiddles.bin", std::ios::binary);
    std::vector<uint64_t> inv_twiddles(num_limbs * N);
    in_tw.read((char*)inv_twiddles.data(), num_limbs * N * sizeof(uint64_t));
    
    // Read inputs (NTT output)
    std::ifstream in_data("../../../integration_tools/test_vectors/tv_ntt_out.bin", std::ios::binary);
    std::vector<uint64_t> data_in(num_limbs * N);
    in_data.read((char*)data_in.data(), num_limbs * N * sizeof(uint64_t));
    
    // Read expected outputs (NTT input)
    std::ifstream out_data("../../../integration_tools/test_vectors/tv_ntt_in.bin", std::ios::binary);
    ...
    for (size_t i = 0; i < num_limbs; i++) {
        ...
        const uint64_t* tw = &inv_twiddles[i * N];
        
        intt_inverse(a.data(), tw, n_inv_mod_q, q, m, k);
        ...
        for(size_t j = 0; j < N; j++) {
            ...
            if (expected != actual) {
                if (errors < 10) {
                    std::cerr << "INTT Error at limb " << i << ", idx " << j 
    ...
    if (errors == 0) {
        std::cout << "SUCCESS: All INTT tests passed bit-accurately!" << std::endl;
    } else {
